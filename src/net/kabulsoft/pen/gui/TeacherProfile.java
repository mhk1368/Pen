...
public class TeacherProfile extends JPanel {
	...
	public TeacherProfile(int id) {
		...
		JLabel title;
		if (id == 0) {
			title = new JLabel("ثبت استاد جدید");
		} else {
			title = new JLabel("ویرایش استاد");
		}
		...
		String texts[] = { "کد استاد:", "نام و تخلص:", "نام پدر:", "نمبر تذکره:", "شماره تلفن:", "آدرس:" };
		...
		if (id == 0) {
			fields[0].setText(String.valueOf(info.maxTeacherId()));
		} else {
			String values[] = info.findTeacher(id);
			for (int i = 0; i < fields.length; i++) {
	...
	private class Register implements ActionListener 
	{
		public void actionPerformed(ActionEvent e) 
		{
			if (Validation.validateData(fields)) {
				...
				if (tcid == 0) {
					if (info.insertTeacher(values)) {
						PenDiags.showMsg("استاد جدید ثبت شد!");
					...
					if (info.editTeacher(values)) {
						PenDiags.showMsg("مشخصات استاد اصلاح شد!");
						info.saveImage(tc_image, tcid);
						if(parent != null){
							parent.renderData();
						}
					}
